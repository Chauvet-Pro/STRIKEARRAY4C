...
2. Connect the DMX out of the STRIKE Array 4C Driver with the latest firmware to the DMX in of the STRIKE Array 4C Driver that needs to be updated.
...
5. A warning ***"make sure no other signal, Network, or DMX controller is being sent! and press enter key to start update"*** will show on the display. Press **< ENTER >** to start the update.
   ...
   >* If the connected product is incorrect or has the incorrect sofware, the upgrade will fail, and the display will go back to the main interface. Repeat **steps 1-5** using a STRIKE Array 4C Driver with valid software.
